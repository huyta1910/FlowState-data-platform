...
const PipelineEditor: React.FC = () => {
  ...
  const addNode = (type: NodeType, subtype?: string, name?: string) => {
    ...
    // Open config modal immediately for sources
    if (type === NodeType.SOURCE) {
      setEditingNode(newNode);
    }
  };

  const handleSaveConfig = (nodeId: string, config: Record<string, string>) => {
    const updatedNodes = activePipeline.nodes.map(n =>
      n.id === nodeId ? { ...n, config, status: 'healthy' as const } : n
    );
    const updatedPipeline = { ...activePipeline, nodes: updatedNodes };
    setPipelines(prev => prev.map(p => p.id === selectedPipelineId ? updatedPipeline : p));
    setEditingNode(null);
  };
                ...
                <div key={node.id} className="flex items-center">
                  ...
                  <div className={`
                    ...
                    <div className="flex justify-between items-center mb-1">
                      <h4 className="font-medium text-slate-200 text-sm truncate max-w-[120px]" title={node.name}>{node.name}</h4>
